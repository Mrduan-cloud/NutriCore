...
```mermaid
flowchart TB
    API["FastAPI · RAG 检索<br/>(营养咨询引用 / 方案证据)"]
    M["Milvus standalone<br/>计算层:向量检索 · 建索引"]
    ETCD["etcd<br/>元数据『账本』<br/>collection / schema / 索引信息<br/>segment 目录 · 节点状态"]
    MINIO["MinIO<br/>对象存储『仓库』<br/>向量数据 · 索引文件 · 日志"]
    ...
    ETCD -.持久化.-> EV[("etcd_data 卷")]
    MINIO -.持久化.-> MV[("minio_data 卷")]
```
